...
const Page = () => {
  ...
  return (
    // Use flex-col on mobile and lg:flex-row on large screens
    <div className='bg-black text-white min-h-[90vh] text-xl p-4 flex flex-col lg:flex-row gap-10'>
      {/* Use w-full on mobile and lg:w-1/2 on large screens */}
      <section className='flex flex-col justify-between w-full lg:w-1/2 bg-gray-900 p-5 rounded-2xl'>
        <EmbeddedStimulation />
        <div className="flex flex-col items-center justify-center px-4 py-2 gap-6 w-full mt-6">
          {/* North–South button - responsive text size */}
          <button
            onClick={() => sendOverride(0)}
            className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-4xl sm:text-5xl lg:text-6xl font-medium text-gray-900 rounded-lg group bg-linear-to-br from-green-400 to-blue-600 hover:text-white dark:text-white w-full"
            disabled = {true}
          >
            <span className="relative px-5 py-2.5 transition-all bg-white dark:bg-gray-900 rounded-md group-hover:bg-transparent w-full text-center">
              North–South
            </span>
          </button>

          {/* East–West button - responsive text size */}
          <button
            onClick={() => sendOverride(1)}
            disabled = {true}
            className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-4xl sm:text-5xl lg:text-6xl font-medium text-gray-900 rounded-lg group bg-linear-to-br from-green-400 to-blue-600 hover:text-white dark:text-white w-full"
      ...
      {/* Use w-full on mobile and lg:w-1/2 on large screens */}
      <section className='flex flex-col justify-between gap-6 w-full lg:w-1/2 bg-gray-900 p-5 rounded-2xl'>
        <TrafficDensityCard
          density={trafficData.density}
          status={trafficData.status}
          history={trafficData.history}
